...
public void НайтиЗапросВЦикле(String ИсходныйКод){
	var ПоискФункций = new Regex(@"(Для|Пока).+Цикл[\S\s]*?КонецЦикла"); //(Для|Пока).+ нужен иначе выражение находит ....Цикла;   код код код для цикл   КонецЦикла;
	//  необходимо переработать это выражение т.к если попадаются вложенные циклы то обрабатываются неверно
